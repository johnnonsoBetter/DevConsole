...
class WebhookServer {
    constructor(outputChannel, chatParticipant) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.server.on("error", (error) => {
                if (error.code === "EADDRINUSE") {
                    ...
                    vscode.window.showErrorMessage(`Webhook server port ${this.port} is in use. Change port in settings.`);
                }
                else {
            ...
            this.server.listen(this.port, () => {
                this.outputChannel.appendLine(`Webhook server listening on http://localhost:${this.port}`);
                vscode.window.showInformationMessage(`Webhook server started on port ${this.port}`);
                resolve();
            });
        });
    }
    ...
    async handleTestRequest(req, res) {
        try {
            ...
            vscode.window.showInformationMessage(`✅ Test request received! (ID: ${testId})`);
        }
        catch (error) {
    ...
    async handleWebhook(req, res) {
        try {
            ...
            if (!workspaceCheck.ready) {
                this.outputChannel.appendLine("Webhook rejected: No workspace folder open");
                res.writeHead(503, { "Content-Type": "application/json" });
                res.end(JSON.stringify(workspaceCheck.error));
                // Also show notification to user in VS Code
                vscode.window
                    .showWarningMessage("Webhook received but no workspace is open. Open a folder to process webhooks.", "Open Folder")
                    .then((choice) => {
            ...
            this.outputChannel.appendLine(`Webhook ${requestId}: chat - ${isQueued
            ...
            res.end(JSON.stringify({
                ...
                message: isQueued
                    ? `Webhook queued at position ${queuePosition + 1}. Another task is currently processing.`
    ...
    async processQueue() {
        ...
        this.outputChannel.appendLine(`Processing webhook: chat [${webhookRequest.id}] (${this.taskQueue.length} remaining in queue)`);
        try {
            let result;
            // If the payload has images and we have a chat participant, use it for multimodal processing
            if (webhookRequest.payload.images &&
                webhookRequest.payload.images.length > 0 &&
                this.chatParticipant) {
                this.outputChannel.appendLine(`Webhook has ${webhookRequest.payload.images.length} image(s), using chat participant for multimodal processing`);
                // Use the chat participant for image processing
                const chatResult = await this.chatParticipant.processWebhookRequest(webhookRequest.id, {
            ...
            this.outputChannel.appendLine(`Webhook ${webhookRequest.id} completed: ${result.success ? "SUCCESS" : "FAILED"}`);
            if (result.success) {
            ...
            if (this.taskQueue.length > 0) {
                this.outputChannel.appendLine(`Queue has ${this.taskQueue.length} more task(s). Processing next in 2 seconds...`);
    ...
    async handleStatusCheck(requestId, res) {
    ...
    updateConfig() {
        ...
        if (newPort !== this.port && this.server) {
            this.outputChannel.appendLine(`Port changed from ${this.port} to ${newPort}, restarting server...`);
            this.port = newPort;
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
            return {
                ...
                error: {
                    success: false,
                    error: "NO_WORKSPACE",
                    message: "No workspace folder is open in VS Code. Please open a project folder first.",
