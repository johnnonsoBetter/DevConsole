...
/**
 * Custom @webhook chat participant
 * Provides:
 * - Direct access to Copilot's language model
 * - Busy state tracking
 * - Streaming response handling
 */

/**
 * Image attachment for chat request
 */
...
export class WebhookChatParticipant {
  ...
  private stateChangeEmitter = new vscode.EventEmitter<ChatState>();
  ...
  async processWebhookRequest(
    ...
    this.outputChannel.appendLine(
      `[Chat Open] Opening Copilot Chat with @webhook participant`
    );
    this.outputChannel.appendLine(`[Chat Open] Request ID: ${requestId}`);
    this.outputChannel.appendLine(
      `[Chat Open] Prompt: ${request.prompt?.substring(0, 100)}${
        request.prompt && request.prompt.length > 100 ? "..." : ""
      }`
    );
    this.outputChannel.appendLine(
      `[Chat Open] Has context: ${!!request.context}`
    );
    this.outputChannel.appendLine(
      `[Chat Open] Query: @webhook ${formattedPrompt.substring(0, 50)}...`
    );
    ...
    this.outputChannel.appendLine(
      `[Chat Open] Successfully opened chat with @webhook participant`
    );
   ...
   */
  private async processWebhookWithImages(
    requestId: string,
    request: ChatRequest
  ): Promise<ChatResult> {
    this.outputChannel.appendLine(
      `Processing webhook with ${
    ...
    try {
      ...
      if (request.images) {
        for (let i = 0; i < request.images.length; i++) {
          ...
          try {
            this.outputChannel.appendLine(
              `Processing image ${i + 1}/${request.images.length}: mimeType=${
                image.mimeType
            ...
            if (!image.data || image.data.length === 0) {
              this.outputChannel.appendLine(
                `Warning: Image ${i + 1} has no data, skipping`
              );
              continue;
            }
            ...
            this.outputChannel.appendLine(
              `Decoded image ${i + 1}: ${bytes.length} bytes`
            );

            // Create image data part
            const imagePart = vscode.LanguageModelDataPart.image(
              bytes,
              image.mimeType
            );
            ...
            if (image.description) {
              messageContent.push(
                new vscode.LanguageModelTextPart(
                  `[Image ${i + 1}: ${image.description}]`
                )
              );
            }
          } catch (error) {
            this.outputChannel.appendLine(
              `Error processing image ${i + 1}: ${error}`
            );
          }
        }
      }
      ...
      this.outputChannel.appendLine(
        `Sending to model: ${messageContent.length} parts (${
          request.images?.length || 0
        } images + text)`
      );
      ...
      this.outputChannel.appendLine(
        `Received response (${fullResponse.length} chars) for webhook: ${requestId}`
      );
      ...
      // Log key info before opening chat
      this.outputChannel.appendLine(
        `[Chat Open] Opening Copilot Chat with image analysis results`
      );
      this.outputChannel.appendLine(
        `[Chat Open] Images processed: ${imageCount}`
      );
      this.outputChannel.appendLine(
        `[Chat Open] Prompt: ${request.prompt?.substring(0, 100)}${
          request.prompt && request.prompt.length > 100 ? "..." : ""
      ...
      this.outputChannel.appendLine(
        `[Chat Open] Response length: ${fullResponse.length} chars`
      );
      ...
      this.outputChannel.appendLine(
        `[Chat Open] Successfully opened chat with image analysis`
      );
      ...
      this.outputChannel.appendLine(
        `Error processing webhook with images: ${error}`
      );
      return {
        success: false,
  ...
  private async callLanguageModel(
    ...
    try {
      ...
      if (images && images.length > 0) {
        this.outputChannel.appendLine(
          `Including ${images.length} image(s) in request`
        );
   ...
   * Set busy state
   */
  private setBusy(isBusy: boolean, requestId: string | null): void {
    this.state = {
      isBusy,
