 ...
 * Responsive: Shows horizontal chips on mobile, sidebar on desktop
 */
...
export function NotesPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      ...
      {/* ================================================================== */}
      {isMobile && (
        <div className="flex flex-col border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800/50 shrink-0">
          {/* Top row: Title, Search, New Note */}
          <div className="flex items-center gap-2 px-3 py-2.5">
              ...
              <div className="flex-1 flex items-center gap-2 max-w-[200px]">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-2 py-1.5 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg border-0 focus:ring-2 focus:ring-primary/20 text-xs placeholder:text-gray-500"
                    autoFocus
                  />
                </div>
                <button
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-xs font-medium transition-all active:scale-95"
          ...
          {/* Note Chips - Horizontally scrollable */}
          <div className="relative">
            {/* Scroll indicators */}
            {filteredNotes.length > 3 && (
              <>
                <div className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-white dark:from-gray-800/50 to-transparent z-10 pointer-events-none" />
                <div className="absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-white dark:from-gray-800/50 to-transparent z-10 pointer-events-none" />
              </>
            )}
            ...
            >
              {isLoading ? (
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500 py-1">
                  ...
                  <span>{filter.search ? 'No notes found' : 'No notes yet'}</span>
                </div>
              ) : (
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? cn(
                              ...
                              colorConfig?.bg || 'bg-primary/10',
                              colorConfig?.border || 'border-primary/30',
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {selectedNote ? (
            <>
              {/* Editor Header */}
              <div className={cn(
                "flex items-center justify-between border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800/50",
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className={cn(
                          "flex-1 px-3 py-2 font-bold bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg border-0 focus:ring-2 focus:ring-primary/20",
                    ...
                    >
                      ...
                      <Edit className="w-4 h-4 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                    </button>
                  )}
                </div>
              ...
              {/* Metadata - Hidden on mobile for space */}
              {!isMobile && (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center max-w-md px-4">
                <div className={cn(
                  "mx-auto mb-6 rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center",
                  isMobile ? "w-16 h-16" : "w-20 h-20"
                )}>
                  <Sparkles className={cn("text-primary", isMobile ? "w-8 h-8" : "w-10 h-10")} />
                </div>
                <h3 className={cn(
                  "font-bold text-gray-900 dark:text-gray-100 mb-2",
                  isMobile ? "text-lg" : "text-xl"
                )}>
                  {isMobile ? 'Tap + to create a note' : 'Start Taking Notes'}
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
                  {isMobile 
                    ? 'Capture ideas and organize with colors.'
                    : 'Capture your ideas, thoughts, and important information. Use rich text formatting, organize with colors, and pin your most important notes.'
                  }
                </p>
                <button
                  onClick={handleCreateNote}
                  className="flex items-center gap-2 px-4 py-2.5 mx-auto bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 text-white rounded-lg font-medium transition-all hover:shadow-apple-sm active:scale-95"
                >
                  <Plus className="w-4 h-4" />
