/**
 * Prompt API Card Component
 * Manages its own state using usePromptModel hook
 */
...
import { usePromptModel } from '../../../hooks/ai/usePromptModel';

export function PromptAPICard() {
  const { 
    availability, 
    checkAvailability, 
    createSession,
  ...
  const handleActivate = async () => {
    ...
    try {
      await createSession('You are a helpful AI assistant.');
      
      // Re-check availability after activation
      await checkAvailability();
    } catch (error) {
      console.error('Failed to activate Prompt API:', error);
    } finally {
