...
export function BetterTabs({ 
  ...
  return (
    <div className={cn("w-full flex flex-col overflow-hidden", className)}>
      {/* Tab List */}
      <div className="relative px-2 sm:px-4 py-2 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-primary/5 to-secondary/5">
        <div
          ref={containerRef}
        ...
        <div 
          className={cn(
            "pointer-events-none absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-gray-50 dark:from-gray-800/50 to-transparent z-10 transition-opacity duration-200",
            containerRef.current?.scrollLeft === 0 ? "opacity-0" : "opacity-100"
          )} 
        />
        <div 
          className={cn(
            "pointer-events-none absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-gray-50 dark:from-gray-800/50 to-transparent z-10 transition-opacity duration-200"
          )} 
        />
      </div>
