...
export function useCallMemory(): UseCallMemoryReturn {
  ...
  const [sessionInfo, setSessionInfo] = useState<MemorySessionInfo | null>(null);
  ...
  useEffect(() => {
    getRaindropSettings().then((settings) => {
      ...
      log('Configuration check:', configured ? 'Configured' : 'Not configured');
    });
  }, []);
    ...
    try {
      ...
      streamRef.current = new TranscriptMemoryStream({
        ...
        onBatchStored: (batchIndex: number, turnCount: number) => {
          ...
          updateSyncStats();
          setSessionInfo((prev: MemorySessionInfo | null) => prev ? {
            ...prev,
        ...
        onError: (err: Error, retriesRemaining: number) => {
          logError('Stream error:', err.message, `(${retriesRemaining} retries left)`);
          updateSyncStats();
      ...
      const message = err instanceof Error ? err.message : 'Failed to start memory session';
  ...
  const endSession = useCallback(async (flush = true): Promise<boolean> => {
    if (!sessionIdRef.current || !workingMemoryRef.current) {
      log('No active session to end');
      return false;
    }

    log('Ending session:', sessionIdRef.current, flush ? '(with flush)' : '(no flush)');
    setState('flushing');
  ...
  const addTranscripts = useCallback((turns: TranscriptTurn[]) => {
    ...
    setTimeout(() => {
      setState((current: MemorySessionState) => current === 'syncing' ? 'connected' : current);
    }, 500);
  }, []);
  ...
  const searchSession = useCallback(async (query: string): Promise<string[]> => {
