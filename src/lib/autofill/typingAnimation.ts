...
export const SPEED_PRESETS: Record<string, Partial<TypingConfig>> = {
  ...
  'instant': { baseDelay: 5, delayVariance: 2, typoChance: 0, speedMultiplier: 10 },
};
...
function randomDelay(base: number, variance: number, multiplier: number): number {
...
async function backspace(
  ...
  const { start: selectionStart } = safeGetSelection(input, currentValue.length);
...
export async function typeWithAnimation(
  ...
  try {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      // Check for typo
      const shouldTypo = Math.random() < finalConfig.typoChance && char.match(/[a-zA-Z0-9]/);
      ...
      let delay = randomDelay(finalConfig.baseDelay, finalConfig.delayVariance, finalConfig.speedMultiplier);
